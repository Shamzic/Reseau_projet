...
unsigned char * create_message_rep_get(unsigned char * hash_file,unsigned char * hash_chunk,char * chunk,short int chunk_index, short int fragment_index, short int max_index,short int nb_cars_lus)
{
    ...
    buffer     = s_int_to_buf(buffer,32,39);
    memcpy(buffer+41,hash_chunk,32);
    buffer = s_int_to_buf(buffer, chunk_index,73);
    // chunk
    buffer[75]  = 60;
